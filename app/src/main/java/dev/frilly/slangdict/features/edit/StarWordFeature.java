...
public final class StarWordFeature implements Runnable {
    ...
    @Override
    public void run() {
        choices.stream().map(Dictionary.getInstance()::getWord).forEach(w -> w.favorite = state);
        if (state)
            Dialogs.info("Marked %d word%s as favorite. These can no longer be deleted.", choices.size(), choices.size() == 1 ? "" : "s");
        else
            Dialogs.info("Unmarked %d word%s from favorite. These can now be deleted.", choices.size(), choices.size() == 1 ? "" : "s");
    }
