...
public final class AddWordFeature implements Runnable {
    ...
    public AddWordFeature(
        String word, String definition, boolean favorite, boolean locked) {
        this.word       = word;
    ...
    @Override
    public void run() {
        if (Dictionary.getInstance().exists(word)) {
            final var opt = Dialogs.confirm(
                "The word \"%s\" already exists. Do you want to OVERWRITE?",
                word);
