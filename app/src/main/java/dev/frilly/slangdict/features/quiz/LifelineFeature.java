...
public final class LifelineFeature implements Runnable {
    ...
    private void runNoah() {
        QuizEventsListener.IMMUNE_INCORRECT = true;
        QuizEventsListener.IMMUNE_CLOCK = true;
        QuizEventsListener.NOAH_ACTIVE = true;
        doAfter(10000, () -> {
            QuizEventsListener.IMMUNE_INCORRECT = false;
            QuizEventsListener.IMMUNE_CLOCK = false;
        });
    }

    // Mio converts 30% damage into raw points.
    private void runMio() {
        QuizEventsListener.MIO_ACTIVE = true;
    }
    ...
    private void runTaion() {
        QuizEventsListener.TAION_ACTIVE = 3;
    }

    // Sena quadruples the next score gain.
    private void runSena() {
        QuizEventsListener.SENA_ACTIVE = true;
    }

    // Lanz reduces score gain by half for the rest of the game, but grants defense against clock and incorrect damages.
    private void runLanz() {
        QuizEventsListener.LANZ_ACTIVE = true;
    }
    ...
    // Nikol reduces damage and clock ticks like Lanz, but combo stack is now disabled.
    private void runNikol() {
        final var state = GameFrame.getInstance().getState();
        state.setComboMultiplier(1.0);
        GameFrame.getInstance().updateDisplay();
        QuizEventsListener.NIKOL_ACTIVE = true;
    }

    // Glimmer heals 10% of Max HP and increases score gain for the next 3 questions.
    private void runGlimmer() {
        final var state = GameFrame.getInstance().getState();
        state.setHp(state.getHp() + state.getMaxHp() * 0.1);
        GameFrame.getInstance().updateDisplay();
        QuizEventsListener.GLIMMER_ACTIVE = 3;
    }
    ...
    private void runRex() {
        ...
        state.setHp(state.getHp() - state.getMaxHp() * 0.1);
        GameFrame.getInstance().updateDisplay();
        QuizEventsListener.REX_ACTIVE = true;
    }

    // Shulk prevents a fatal damage blow.
    private void runShulk() {
        QuizEventsListener.SHULK_ACTIVE = true;
    }
